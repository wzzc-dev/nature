...
#define TEST_EQ(inst, ...) ({                                                        \
    uint8_t *data = riscv64_asm_inst_encoding(inst);                                 \
...
static void test_basic() {
    ...
    // rs, rs1, imm
    inst = RISCV64_INST(R_ADDI, RO_REG(T0), RO_REG(T0), RO_IMM(1));
    TEST_EQ(inst, 0x85, 0x02);

    inst = RISCV64_INST(R_ADDI, RO_REG(T2), RO_REG(T0), RO_IMM(1));
    TEST_EQ(inst, 0x93, 0x83, 0x12, 0x00);

    inst = RISCV64_INST(R_ADD, RO_REG(T0), RO_REG(T1), RO_REG(T2));
    TEST_EQ(inst, 0xB3, 0x02, 0x73, 0x00);

    inst = RISCV64_INST(R_SUB, RO_REG(T0), RO_REG(T1), RO_REG(T2));
    TEST_EQ(inst, 0xB3, 0x02, 0x73, 0x40);

    inst = RISCV64_INST(R_ADDIW, RO_REG(T0), RO_REG(T1), RO_IMM(10));
    TEST_EQ(inst, 0x9B, 0x02, 0xA3, 0x00);

    inst = RISCV64_INST(R_LD, RO_REG(T1), RO_INDIRECT(SP, 12, QWORD));
    TEST_EQ(inst, 0x03, 0x33, 0xC1, 0x00);

    inst = RISCV64_INST(R_SD, RO_REG(T2), RO_INDIRECT(T0, 304, QWORD));
    TEST_EQ(inst, 0x23, 0xB8, 0x72, 0x12);

    inst = RISCV64_INST(R_BEQ, RO_REG(A0), RO_REG(A1), RO_IMM(0xc));
    TEST_EQ(inst, 0x63, 0x06, 0xB5, 0x00);

    inst = RISCV64_INST(R_BEQ, RO_REG(T0), RO_REG(T1), RO_SYM("end", true, 0, 0));
    TEST_EQ(inst, 0x63, 0x80, 0x62, 0x00);

    inst = RISCV64_INST(R_BNE, RO_REG(A0), RO_REG(A1), RO_IMM(0xa));
    TEST_EQ(inst, 0x63, 0x15, 0xB5, 0x00);

    inst = RISCV64_INST(R_CALL, RO_SYM("foo", false, 0, 0));
    TEST_EQ(inst, 0x97, 0x00, 0x00, 0x00, 0xE7, 0x80, 0x00, 0x00);

    inst = RISCV64_INST(R_LI, RO_REG(T0), RO_IMM(12));
    TEST_EQ(inst, 0xB1, 0x42);

    inst = RISCV64_INST(R_MV, RO_REG(T1), RO_REG(T2));
    TEST_EQ(inst, 0x1E, 0x83);

    inst = RISCV64_INST(R_FLD, RO_REG(FT0), RO_INDIRECT(T0, 0, QWORD));
    TEST_EQ(inst, 0x07, 0xB0, 0x02, 0x00);

    inst = RISCV64_INST(R_FADD_S, RO_REG(FT0), RO_REG(FT1), RO_REG(FT2));
    TEST_EQ(inst, 0x53, 0xf0, 0x20, 0x00); 

    inst = RISCV64_INST(R_FSUB_S, RO_REG(FT0), RO_REG(FT1), RO_REG(FT2));
    TEST_EQ(inst, 0x53, 0xf0, 0x20, 0x08);

    inst = RISCV64_INST(R_FMUL_S, RO_REG(FT0), RO_REG(FT1), RO_REG(FT2));
    TEST_EQ(inst, 0x53, 0xf0, 0x20, 0x10);

    inst = RISCV64_INST(R_FDIV_S, RO_REG(FT0), RO_REG(FT1), RO_REG(FT2));
    TEST_EQ(inst, 0x53, 0xf0, 0x20, 0x18);

    inst = RISCV64_INST(R_FCVT_S_W, RO_REG(FT0), RO_REG(T0));
    TEST_EQ(inst, 0x53, 0xf0, 0x02, 0xD0);

    inst = RISCV64_INST(R_FCVT_W_S, RO_REG(T0), RO_REG(FT0)); // Assuming T0 is dest, FT0 is src
    TEST_EQ(inst, 0xd3, 0x72, 0x00, 0xC0); // Expected: C0080253 (rd=t0, rs1=ft0, rm=000)

    inst = RISCV64_INST(R_FLW, RO_REG(FT0), RO_INDIRECT(T0, 0, DWORD));
    TEST_EQ(inst, 0x07, 0xA0, 0x02, 0x00);

    inst = RISCV64_INST(R_FSW, RO_REG(FT0), RO_INDIRECT(T0, 0, DWORD));
