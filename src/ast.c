...
static ast_select_stmt_t *ast_select_copy(module_t *m, ast_select_stmt_t *temp);
...
static type_chan_t *type_chan_copy(module_t *m, type_chan_t *temp) {
    type_chan_t *t = COPY_NEW(type_chan_t, temp);
    t->element_type = type_copy(m, temp->element_type);
    return t;
}
...
type_t type_copy(module_t *m, type_t temp) {
    ...
    if (temp.ident) {
        ...
        type.args = temp.args;
        if (temp.args) {
            type.args = ct_list_type_copy(m, temp.args);
        }
    }
    ...
    switch (temp.kind) {
        ...
        case TYPE_CHAN: {
            type.chan = type_chan_copy(m, temp.chan);
            break;
        }
        case TYPE_ARR: {
...
static ast_new_expr_t *ast_new_expr_copy(module_t *m, ast_new_expr_t *temp) {
    ...
    if (temp->default_expr) {
        new_expr->default_expr = ast_expr_copy(m, temp->default_expr);
    }
...
static ast_array_new_t *ast_arr_new_copy(module_t *m, ast_array_new_t *temp) {
    ast_array_new_t *arr_new = COPY_NEW(ast_array_new_t, temp);
    if (temp->elements) {
        arr_new->elements = ast_list_expr_copy(m, temp->elements);
    }
...
static ast_vec_new_t *ast_vec_new_copy(module_t *m, ast_vec_new_t *temp) {
...
static ast_vec_repeat_new_t *ast_vec_repeat_new_copy(module_t *m, ast_vec_repeat_new_t *temp) {
    ast_vec_repeat_new_t *vec_new = COPY_NEW(ast_vec_repeat_new_t, temp);
    vec_new->default_element = *ast_expr_copy(m, &temp->default_element);
    vec_new->length_expr = *ast_expr_copy(m, &temp->length_expr);
...
static ast_vec_slice_t *ast_vec_slice_copy(module_t *m, ast_vec_slice_t *temp) {
    ast_vec_slice_t *slice = COPY_NEW(ast_vec_slice_t, temp);
    slice->left = *ast_expr_copy(m, &slice->left);
    slice->start = *ast_expr_copy(m, &slice->start);
    slice->end = *ast_expr_copy(m, &slice->end);
    return slice;
}
...
static ast_expr_select_t *ast_select_expr_copy(module_t *m, ast_expr_select_t *temp) {
...
static ast_macro_async_t *ast_async_copy(module_t *m, ast_macro_async_t *temp) {
    ast_macro_async_t *expr = COPY_NEW(ast_macro_async_t, temp);
    expr->args_copy_stmts = ast_body_copy(m, expr->args_copy_stmts);
...
ast_expr_t *ast_expr_copy(module_t *m, ast_expr_t *temp) {
    ...
    switch (temp->assert_type) {
        ...
        case AST_EXPR_VEC_SLICE: {
            expr->value = ast_vec_slice_copy(m, temp->value);
            break;
        }
        case AST_EXPR_VEC_NEW: {
            expr->value = ast_vec_new_copy(m, temp->value);
            break;
        }
        case AST_EXPR_ARRAY_NEW: {
            expr->value = ast_arr_new_copy(m, temp->value);
            break;
        }
        case AST_EXPR_ARRAY_REPEAT_NEW:
        case AST_EXPR_VEC_REPEAT_NEW: {
            expr->value = ast_vec_repeat_new_copy(m, temp->value);
            break;
        }
        ...
        case AST_MACRO_ASYNC: {
            expr->value = ast_async_copy(m, temp->value);
            break;
        }
        case AST_FNDEF: {
        ...
        case AST_EXPR_SELECT: {
            expr->value = ast_select_expr_copy(m, temp->value);
            break;
        }
        default:
...
static ast_select_stmt_t *ast_select_copy(module_t *m, ast_select_stmt_t *temp) {
    ...
    SLICE_FOR(temp->cases) {
        ...
        new_select_case->on_call = ast_call_copy(m, match_case->on_call);
        if (match_case->recv_var) {
            new_select_case->recv_var = ast_var_decl_copy(m, match_case->recv_var);
        }
        new_select_case->handle_body = ast_body_copy(m, match_case->handle_body);
...
static ast_catch_t *ast_catch_copy(module_t *m, ast_catch_t *temp) {
    ...
    catch->catch_body = ast_body_copy(m, temp->catch_body);
    return catch;
}

static ast_try_catch_stmt_t *ast_try_catch_stmt_copy(module_t *m, ast_try_catch_stmt_t *temp) {
    ast_try_catch_stmt_t *try_stmt = COPY_NEW(ast_try_catch_stmt_t, temp);
    try_stmt->try_body = ast_body_copy(m, temp->try_body);
    try_stmt->catch_err = *ast_var_decl_copy(m, &temp->catch_err);
    try_stmt->catch_body = ast_body_copy(m, temp->catch_body);
    return try_stmt;
}
...
static ast_var_tuple_def_stmt_t *ast_var_tuple_def_copy(module_t *m, ast_var_tuple_def_stmt_t *temp) {
    ...
    stmt->tuple_destr = ast_tuple_destr_copy(m, temp->tuple_destr);
    stmt->right = *ast_expr_copy(m, &temp->right);
    return stmt;
}

static ast_assign_stmt_t *ast_global_assign_copy(module_t *m, ast_global_assign_stmt_t *temp) {
...
static ast_if_stmt_t *ast_if_copy(module_t *m, ast_if_stmt_t *temp) {
    ...
    stmt->consequent = ast_body_copy(m, temp->consequent);
    stmt->alternate = ast_body_copy(m, temp->alternate);
    return stmt;
}
...
static ast_call_t *ast_call_copy(module_t *m, ast_call_t *temp) {
    ...
    call->generics_args = ct_list_type_copy(m, call->generics_args);
...
static ast_stmt_t *ast_stmt_copy(module_t *m, ast_stmt_t *temp) {
    ...
    switch (temp->assert_type) {
        ...
        case AST_STMT_GLOBAL_ASSIGN: {
            stmt->value = ast_global_assign_copy(m, temp->value);
            break;
        }
        case AST_STMT_IF: {
        ...
        case AST_STMT_SELECT: {
            stmt->value = ast_select_copy(m, temp->value);
            break;
        }
        case AST_MATCH: {
            stmt->value = ast_match_copy(m, temp->value);
            break;
        }
        case AST_STMT_TRY_CATCH: {
            stmt->value = ast_try_catch_stmt_copy(m, temp->value);
            break;
        };
        default:
